...
public class CemBoidBase : MonoBehaviour
{
    ...
    void manageCameraRotation()
    {
        if(m_isFreeze && !s_cameraRotated && s_moveCameraWhileIdle)
        {
            ...
            //Constants.getPlayer().GetComponent<CameraRotation>().currentAngleY += s_cameraRotationSpeed * Time.deltaTime * Mathf.Cos(Constants.getPlayer().GetComponent<CameraRotation>().currentAngleY / 30) * Mathf.Cos(Time.time * 0.2f);
    ...
    public void setSwarmSize(int number)
    {
        ...
        while(m_agents.Count != number && tries < 10000)
        {
            if(m_agents.Count < number)
            {
                ...
                Constants.getPlayer().GetComponent<PlayerEntityAttachSystem>().addToGrab(agent, false, true);
    ...
    public void removeAgent(GameObject agent)
    {
        ...
        {
            if(agent == null)
                agent = m_agents[Random.Range(0, m_agents.Count)];
    ...
    // utility
    public void updateShowGrabbedText()
    {
        if (m_showGrabbedObject != null && m_showGrabbedObject.GetComponent<Text>() != null)
            m_showGrabbedObject.GetComponent<Text>().text = m_agents.Count.ToString();
    }
