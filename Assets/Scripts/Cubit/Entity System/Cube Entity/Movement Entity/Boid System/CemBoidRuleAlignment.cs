...
public class CemBoidRuleAlignment : CemBoidRuleBase
{
    [Header("------- Settings -------")]
    ...
    Dictionary<GameObject, float> m_alignmentAcualRadii;
    ...
    void initializeStuff()
    {
        ...
        m_alignmentAcualRadii = new Dictionary<GameObject, float>();
    }
    ...
    void getAlignmentForceVector(GameObject agent, List<GameObject> agents)
    {
        ...
        Collider[] colliders = Physics.OverlapSphere(agent.transform.position, m_alignmentAcualRadii[agent]);
        // adjust radius
        if (m_useAdjustRadius && m_alignmentMaxPartnerChecks > 0)
        {
            if (colliders.Length - m_alignmentMaxPartnerChecks > m_alignmentMinAdjustmentDifference)
            {
                m_alignmentAcualRadii[agent] -= m_alignmentAdjustStep;
            }
            else if (colliders.Length - m_alignmentMaxPartnerChecks < -m_alignmentMinAdjustmentDifference)
            {
                m_alignmentAcualRadii[agent] += m_alignmentAdjustStep;
            }
            m_alignmentAcualRadii[agent] = Mathf.Clamp(m_alignmentAcualRadii[agent], m_alignmentMinRadius, m_alignmentRadius);
        }
    ...
    public override void onAddAgent(List<GameObject> agents, GameObject agent)
    {
        if (!m_alignmentForceVectors.ContainsKey(agent))
        {
            ...
            m_alignmentAcualRadii.Add(agent, m_alignmentRadius);
        }
        else
    ...
    public override void onRemoveAgent(List<GameObject> agents, GameObject agent)
    {
        if (m_alignmentForceVectors.ContainsKey(agent))
        {
            ...
            m_alignmentAcualRadii.Remove(agent);
        }
        else
    ...
    {
        List<GameObject> agnets = new List<GameObject>(m_alignmentAcualRadii.Keys);
        foreach (GameObject agent in agnets)
            m_alignmentAcualRadii[agent] = m_alignmentRadius;
    }
    ...
    public override void setValues(CemBoidRuleBase copyScript)
    {
        ...
        agnets = new List<GameObject>(m_alignmentAcualRadii.Keys);
        foreach (GameObject agent in agnets)
            m_alignmentAcualRadii[agent] = m_alignmentRadius;
    }
}
