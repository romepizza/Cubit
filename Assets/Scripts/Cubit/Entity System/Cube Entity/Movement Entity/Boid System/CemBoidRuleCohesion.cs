...
public class CemBoidRuleCohesion : CemBoidRuleBase
{
    [Header("------- Settings -------")]
    ...
    void getCohesionForceVector(GameObject agent, List<GameObject> agents)
    {
        ...
        if (m_cohesionRadius <= 0)
        {
            foreach (GameObject agent2 in agents)
            {
                ...
                // check for angle
                if (m_requireAngle && getAngle(agent2.transform.position - agent.transform.position, agent.transform.forward) > m_maxAngle)
                    continue;
