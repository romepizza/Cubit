...
public class CemBoidRulePredator : CemBoidRuleBase {
    ...
    void getPredatorForceVector(List<GameObject> agents)
    {
        ...
        { 
            ...
            foreach (Collider collider in colliders)
            {
                ...
                // check for angle
                if (m_requireAngle && getAngle(collider.transform.position - predator.transform.position, predator.transform.forward) > m_maxAngle)
                    continue;
