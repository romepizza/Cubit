...
public class GrabSystem : MonoBehaviour
{
        ...
        for (int i = 0; i < grabbedCubes.Count; i++)
        {
            ...
            if (!logged)
            {
                if (GameObject.Find("LogInformationObject") != null && GameObject.Find("LogInformationObject").GetComponent<Log>() != null)
                    GameObject.Find("LogInformationObject").GetComponent<Log>().logRegisterSkillPush(grabbedCubes.Count, true, isAim);
                else if (GameObject.Find("GeneralScriptObject").GetComponent<Log>())
                    GameObject.Find("GeneralScriptObject").GetComponent<Log>().logRegisterSkillPush(grabbedCubes.Count, true, isAim);
                logged = true;
            }
            pushedFromGrabTotal++;
    ...
    public void launchCubesSuccessively(float duration, float power, float maxSpeed, float activeTime, LayerMask layerMask, Vector3 offset, bool aimHelpActivated)
    {
        if (!isLaunching)
        {
            ...
            if (GameObject.Find("LogInformationObject") != null && GameObject.Find("LogInformationObject").GetComponent<Log>() != null)
                GameObject.Find("LogInformationObject").GetComponent<Log>().logRegisterSkillPush(shootCubesTotal, true, isAim);
            else if (GameObject.Find("GeneralScriptObject").GetComponent<Log>())
                GameObject.Find("GeneralScriptObject").GetComponent<Log>().logRegisterSkillPush(shootCubesTotal, true, isAim);
    ...
    void Update ()
    {
       ...
       // Debug.Log("1: " + GameObject.Find("GeneralScriptObjct") != null + GameObject.Find("GerenalScriptObject").GetComponent<Log>().logPlayerSummary);
       // Debug.Log("2: " + waveSystem.GetComponent<MonsterManager>().wavesIsActive + !waveSystem.GetComponent<MonsterManager>().isFreeze + !waveSystem.GetComponent<MonsterManager>().waitingForWave);

        if (GameObject.Find("GeneralScriptObject") != null && GameObject.Find("GeneralScriptObject").GetComponent<Log>().logPlayerSummary)
        {
            if(waveSystem.GetComponent<MonsterManager>().wavesIsActive && !waveSystem.GetComponent<MonsterManager>().isFreeze && !waveSystem.GetComponent<MonsterManager>().waitingForWave)
                logGrabbed();
        }
    }
}
