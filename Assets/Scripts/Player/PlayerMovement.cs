...
public class PlayerMovement : MonoBehaviour
{
    ...
    void applyForceVector()
    {
       ...
       // Debug.DrawRay(transform.position + Vector3.up * 2f, rb.velocity, Color.yellow);
        ...
        if(affectCubes && GetComponent<GrabSystem>() != null)
        {
            foreach (GameObject cube in GetComponent<GrabSystem>().grabbedCubes)
            {
                cube.GetComponent<Rigidbody>().AddForce((forceMovementVectorTotal + airRestianceVectorTotal) * affectFactor, ForceMode.Acceleration);
            }
        }
