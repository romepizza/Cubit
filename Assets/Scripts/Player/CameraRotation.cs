...
public class CameraRotation : MonoBehaviour
{
	...
	void LateUpdate ()
    {
        ...
        /*
        if (GetComponent<Aim>() != null && GetComponent<Aim>().m_isLocked)
        {
            Quaternion rotationBefore = playerCamera.transform.rotation;
            playerCamera.transform.LookAt(GetComponent<Aim>().m_aimingAt.transform.position);


            currentAngleX = playerCamera.transform.rotation.eulerAngles.y;
            currentAngleY = playerCamera.transform.rotation.eulerAngles.x;
            if (currentAngleY > 180)
                currentAngleY -= 360;
            currentAngleY = Mathf.Clamp(currentAngleY, -80, 80);
            //float zBefore = rotationBefore.eulerAngles.z - playerCamera.transform.rotation.eulerAngles.z;
        ...
        */
        //else
        {
            cameraRotation = Quaternion.Euler(currentAngleY, currentAngleX, 0);
        }
